...
# This Rscript generates a months worth of videos for a South Coast community.
# Where createVideo_exec.R creates daily updates, this script backfills with
# by creating an entire month's worth of videos and can take an hour or more to
# run.
#
...
monthStamp <- 
  ...
  (start + lubridate::ddays(40)) %>% 
  lubridate::floor_date(unit = "months")

datestamps <- 
  seq(start, end, by = "day") %>% 
  strftime("%Y%m%d") %>%
  stringr::str_subset(opt$datestamp)
...
for ( datestamp in datestamps ) {
  ...
  result <- try({ 
    ...
    # Get date range 
    # NOTE:  We use 'ceilingEnd' here because sensorLoad() trims to day
    # NOTE:  boundaries and ends just before 'enddate'.
...
# Handle errors
if ( "try-error" %in% class(result) ) {
  msg <- paste("Error creating video archive: ", geterrmessage())
  logger.fatal(msg)
} else {
  # Guarantee that the errorLog exists
  if ( !file.exists(errorLog) ) dummy <- file.create(errorLog)
